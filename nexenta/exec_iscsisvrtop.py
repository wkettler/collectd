...
collectd EXEC wrapper for the modified version of iscsisvrtop.
...
# Start subprocess
producer = subprocess.Popen(["/usr/local/collectd/bin/iscsisvrtop",
...
while True:
    ...
    sys.stdout.write('PUTVAL "%s/iscsisvrtop/%s" %s:%s\n' % (hostname, key,
                                                             time, value))
    sys.stdout.flush()
